		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->sensor_sleep && rmi4_data->enable_wakeup_gesture) {
		...
		if (retval < 0)
			return 0;
        TP_DBG("detected_gestures:%x,%x,%x,%x==\n",detected_gestures[0],detected_gestures[1],detected_gestures[2],detected_gestures[3]);
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	else
	{
	    if (rmi4_data->enable_wakeup_gesture) 
                ...
                msleep(200);
                TP_DBG("============synaptics_rmi4_resume===set_tgesture_unenable==\n");	
	}
