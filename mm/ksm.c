...
static unsigned int ksm_thread_sleep_millisecs = 700;//20;
...
static void ksm_tuning_pressure(void)
{
	...
	if (bat_is_charger_exist() == KAL_TRUE) {
		if (ksm_thread_sleep_millisecs == 700/*20*/ &&
			ksm_thread_pages_to_scan == 100)
			return;
		/*set to default value */
		ksm_thread_sleep_millisecs = 700;//20;
		ksm_thread_pages_to_scan = 100;
	} else {
...
#ifdef CONFIG_PROJECT_P6601_BLU_US
        unsigned long totalMemory;
        totalMemory=totalram_pages << PAGE_SHIFT;
